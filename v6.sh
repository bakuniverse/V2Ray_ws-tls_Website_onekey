...
apache_uninstall(){
	...
	rm -rf /etc/systemd/system/v2ray.service /usr/local/etc/v2ray/config.json /usr/local/etc/v2ray/user.json >/dev/null 2>&1
...
modify_crontab(){
	...
	echo "20 14 * * * bash /root/v2ray/install-release.sh | tee -a /root/v2ray/update.log && service v2ray restart >/dev/null 2>&1" >> crontab.txt
...
nginx_conf_add(){
	...
	server {
		...
		ssl_trusted_certificate /usr/local/etc/v2ray/v2ray.crt;
...
main(){
if [[ -e /usr/local/etc/v2ray/v2ray.key ]]; then
