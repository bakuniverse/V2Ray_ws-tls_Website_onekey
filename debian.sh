...
random_number(){
	...
	camouflage=`cat /dev/urandom | head -n 10 | md5sum | head -c 7`
	hostheader=`cat /dev/urandom | head -n 10 | md5sum | head -c 5`
}
...
dependency_install(){
	...
	${INS} install curl cron lsof unzip zip gcc make libpcre3 libpcre3-dev zlib1g-dev libldap2-dev libssl-dev -y
...
nginx_install(){
  ...
  sed -i 's@CFLAGS="$CFLAGS -g"@#CFLAGS="$CFLAGS -g"@' ${nginx_openssl_src}/nginx-${nginx_version}/auto/cc/gcc
    sleep 4
    ...
    # 修改基本配置
    sed -i 's/#user  nobody;/user  www;/' ${nginx_dir}/nginx.conf
    sed -i '$i include /etc/nginx/conf/*.conf;' ${nginx_dir}/nginx.conf
...
show_information(){
	...
	echo -e "${Green} WS 路径（ws  path）（Path）（WebSocket 路径）：${Font} /${hostheader} "
	echo -e "${Green} WS Host（伪装域名）（Host）：${Font} www.${hostheader}.com"
	echo -e "${Green} 伪装域名（适用于 旧版v2rayNG）：${Font} ${Font} ${domain}"
	echo -e "${Green} HTTP头（适用于 BifrostV）：${Font} 字段名：host 值：${Font} ${domain}"
